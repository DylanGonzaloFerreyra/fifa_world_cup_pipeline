...
FROM (
    SELECT winners AS country FROM fifa_world_cup_finals
    UNION ALL
    SELECT runners_up AS country FROM fifa_world_cup_finals
) AS all_finalists
GROUP BY country
...
FROM (
    SELECT winners AS country, CAST(winner_goals AS INT) AS goals FROM fifa_world_cup_finals
    UNION ALL
    SELECT runners_up AS country, CAST(runner_up_goals AS INT) AS goals FROM fifa_world_cup_finals
) AS all_goals
GROUP BY country
